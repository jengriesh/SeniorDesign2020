...
%   This is a CELL MODE script.  Don't try running it straight. 
...
RGBlock=1;           % perform spatial dithering on channels non-independently
opacity=1;           % opacity of block layers (pertains to blendmode)
blendmode='posterize';  % see 'help imblend'
blendamount=1;       % amount parameter passed to blend function (used only for dodge/burn/permute)
...
%inpict=fourdee(@imresize,inpict,0.33);
...
im2spectrogram(inpict,outpath,alteraspect,padbar,volume,[blurrad bluramt]);
...
text2spectrogram(instring,outpath,textheight,textlocation,alteraspect,volume,[blurrad bluramt]);
...
h=fspecial('disk',5);
bspots=imfilter(randspots(s,numspots/2,spotbounds,spotopacity,shape,fill),h);
...
lines=imfilter(eoline(grad,1,[5 15]),h);
...
bg=imread([projdir 'sources/allthelights.jpg'], 'jpeg');

s=size(bg);
spots=randspots(s,100,[50 100],[0 0.3],'rectangle');
fg=imlnc(randlines(size(bg),2,0.95,1,'walks',0.2));
fg=imblend(spots,fg,1,'permute dh>h',0.1);

mask=imlnc(randlines(size(bg),1,0.98,1,'walks',10));
mask=imblend(flipdim(flipdim(spots,2),1),mask,1,'overlay',1);
...
field=imlnc(randlines(size(inpict),direction,0.915736,round(0.792207),'walks',rate)); 
...
field=dilatemargins(inpict,ceil([0.338132 0.832334]*20),se); 
