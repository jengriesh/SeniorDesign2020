...
% a lot of these are configured for viewing on an inverted display

%% colorpict demo %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

s=[200 300];
c=[255 128 0];
outpict=colorpict(s,c);
...
%outpict=padarray(inpict,[25 25],0);
%imshow(outpict)
%imwrite(outpict,'examples/cropborderex1.jpg','jpeg','Quality',90);

inpict=imread([projdir 'examples/cropborderex1.jpg']);
...
se=strel('disk',5);
outpict=dilatemargins(inpict,[1 1]*20,se,0,'rgb');
...
clc; clear all;
projdir='/data/homebak/cad_and_projects/imagepooper/';
inpict=imresize(imread([projdir 'sources/blacklight2.jpg']),0.5);
...
%gifwrite(outpict,[projdir 'examples/driftdsex4.gif']);
imshow(outpict(:,:,:,1))

%% eoline demo %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
inpict=imread([projdir 'sources/goat.jpg']);
...
%% findpixels demo %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
inpict=imread([projdir 'sources/colorballs.jpg']);
...
outpict=fourdee(@imresize,inpict,0.4);
...
%% imrange demo %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
inpict=imread([projdir 'sources/blacklight2.jpg']);
...
%%
inpict=imread([projdir 'sources/blacklight2.jpg']);
m=eoline(colorpict(size(inpict),[1 1 1]*255),1,[10 25]);
mask=replacepixels([255 0 0],colorpict(size(inpict),[0 0 255]),m);
...
outpict=meanlines(inpict,2,1,'mean');
...
outpict=meanlines(inpict,1,1,'max y');
...
inpict=imread([projdir 'sources/bananas.jpg'], 'jpeg');
...
%% 
inpict=imread([projdir 'examples/jpeggerex1.jpg'], 'jpeg');
...
outpict=imblend(meanlines(slurpict,1,0),outpict,1,'contrast',0.7);
...
s=[600 200 3];
mode='normal';
A=randlines(s,1,0,0,mode);
C=randlines(s,1,0,1,mode);
B=randlines(s,1,0.90,0,mode);
D=randlines(s,1,0.90,1,mode);
...
mode='walks';
A=randlines(s,1,0,0,mode);
B=randlines(s,1,0.95,0,mode);
C=randlines(s,1,0,1,mode);
D=randlines(s,1,0.95,1,mode);
outpict=cat(2,A,B,C,D);
...
s=[600 200 3];
mode='walks';
A=imlnc(randlines(s,1,0,0,mode));
B=imlnc(randlines(s,1,0.95,0,mode));
C=imlnc(randlines(s,1,0,1,mode));
D=imlnc(randlines(s,1,0.95,1,mode));
outpict=cat(2,A,B,C,D);
...
s=[600 200 3];
mode='ramps';
A=randlines(s,1,0,0,mode,3);
B=randlines(s,1,0,0,mode,1);
C=randlines(s,1,0,1,mode,3);
D=randlines(s,1,0,1,mode,1);
outpict=cat(2,A,B,C,D);
...
inpict=imread('sources/goat.jpg');
se=strel('disk',2);
mask=imopen(rangemask(inpict,'sv',[0 0.5; 0 0.5]*255),se);
outpict=thresholdinpaint(inpict,'rgb',mask,0,0);
imshow(outpict)
%% roiflip demo %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
s=size(mask);
%inpict=imlnc(randlines([s 3],2,0.95,1,'walks',0.2));

cont=roiflip(inpict,mask,2,1); 
disc=roiflip(inpict,mask,2,0);
...
amt=40;
fill='circular';
cont=roishift(inpict,mask,2,amt,1,fill); 
disc=roishift(inpict,mask,2,amt,0,fill);
...
outpict=padarray(inpict,[1 1]*srad,'replicate');
...
tiles=[1 1]*5;
outpict=shuffle(inpict,tiles,1);
...
tiles=[1 1]*5;
outpict=shuffle(inpict,tiles,0);
...
outpict=shuffle(inpict,tiles,0,perms);
...
fill=[0 255 0];
bg=imlnc(randlines([s 3],2,0.95,1,'walks',0.2));
...
flines=imlnc(randlines([s 3],1,0.95,1,'walks',0.2));
...
blines=imlnc(randlines([s 3],2,0.95,1,'walks',0.2));
...
outpict=imecho(inpict,5,'screen');

%gifwrite(outpict, 'examples/imechoex1.gif')
%% 
inpict=gifread('examples/mergedownex1.gif');

outpict=imecho(inpict,5,'screen',2);

%gifwrite(outpict, 'examples/imechoex2.gif')
%% 
inpict=gifread('examples/mergedownex1.gif');

outpict=imecho(inpict,0,'screen',1,[0 2 4]);

%gifwrite(outpict, 'examples/imechoex3.gif')
%% 
inpict=gifread('examples/mergedownex1.gif');

outpict=imecho(inpict,5,'screen',2,[0 0 0],'blocks');
...
midcolor=colorpict(s,[1 1 1]*128);
...
outpict=tonemap(inpict,100,0.9,0.6);

imshow(255-cat(2,inpict,outpict))
...
out=[];
for h=0:1/steps:1;
    ch=colorpict(round(s./[2 steps]),imtweak(c0,mode,[h 1 1]));
    chp=colorpict(round(s./[2 steps]),imtweak(c0,modep,[h 1 1]));
    ch=cat(1,ch,chp);
    out=cat(2,out,ch);
end
...
out=imresize(cat(1,hslrgb,rgb,rgbp),[750 500],'nearest');
...
gamut=cat(1,flipdim(gamutb,1),flipdim(gamuty,1));
...
catpict=imresize(cat(1,inpict(:,:,:,1),inpict(:,:,:,2),inpict(:,:,:,3),inpict(:,:,:,4)),[1200 NaN]);
