...
function figuresetup()
	...
	'Style','checkbox',...
	'value',rainbow,...
	'TooltipString','decreases channel correlation',...
	'Tag','rainbowcheckbox',...
	'callback',{@setparam,'rainbow'});
	
	uicontrol(...
	'Parent',ep8,...
	'Units','normalized',...
	'FontSize',10,...
	'Position',[0.11284046692607 0.939-0.12 0.88 0.029535864978903],...
	'String','Enhance Contrast',...
	'Style','checkbox',...
	'value',contrastenhanced,...
...
function previewbutton_CBF(~,~)
	...
	switch mode
		...
		case 'checkerboard' 
			...
			mask=xor(mod(xx,width(1)*2)<width(1),mod(yy,height(1)*2)<height(1));
		...
		case 'random lines'
			if contrastenhanced
				protoimage=imlnc(imcast(randlines([s 3],thisdim,sparsity, ...
					colorenabled,randmodestrings{thisrandmode},rate),'double'));
			else
				protoimage=imcast(randlines([s 3],thisdim,sparsity, ...
					colorenabled,randmodestrings{thisrandmode},rate),'double');
			end
		case 'random shapes'
		...
		case 'clouds'
			if contrastenhanced
				protoimage=imlnc(imcast(perlin([s 3],rainbow),'double'));
			else
				protoimage=imcast(perlin([s 3],rainbow),'double');
			end
			if ~colorenabled
