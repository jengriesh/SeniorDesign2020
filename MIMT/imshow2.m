...
elseif isempty(inputimage) && ~isempty(inputimagestring)
	try
		...
		fprintf('IMSHOW2: Invalid image ''%s''\n',inputimagestring);
	end
end
...
function figuresetup()
	...
	'Style','edit',...
	'TooltipString','number of intensity levels to sample',...
...
function updateranges()
	...
	for c=1:size(imagetoshow,3)
		chanrange=imrange(imagetoshow(:,:,c));
		nancount=sum(sum(isnan(imagetoshow(:,:,c))));
		oog=sum(sum(imagetoshow(:,:,c)<0)) + sum(sum(imagetoshow(:,:,c)>1));

		set(eval(sprintf('handles.chanrange%d',c)),'visible','on')
		set(eval(sprintf('handles.chanrange%d',c)),'string',mat2str(chanrange,6))
		...
		set(eval(sprintf('handles.oogcount%d',c)),'string',mat2str(oog))
	end
	
	for c=(size(imagetoshow,3)+1):4
		set(eval(sprintf('handles.chanrange%d',c)),'visible','off')
...
function safepict=alphize(thispict)
	if mod(size(thispict,3),2)==0
		...
		% avoid imblend dependency
		mat=repmat(mat,[1 1 size(thispict,3)-1]);
		FGA=thispict(:,:,end);
		safepict=bsxfun(@times,FGA,thispict(:,:,1:end-1)) + bsxfun(@times,(1-FGA),mat);
	else
		safepict=thispict;
	end
end
...
function prepimage()
	...
	imagetoshow=imcast(imagetoshow,'double');
