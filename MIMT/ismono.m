...
hasalpha=1-mod(size(inpict,3),2);
numcolorchans=size(inpict,3)-hasalpha;

if numcolorchans==1
    sureis=true;
else
	wpict=inpict(:,:,1:numcolorchans,:);
	
    % method similar to ISGRAY(); 
    % test small area for possible early return
    maxpatch=[100 100];
    testpatch=min([size(wpict,1) size(wpict,2)],maxpatch);
	diffpart=abs(diff(imcast(wpict(1:testpatch(1),1:testpatch(2),:,:),'double'),1,3));
    apd1=sum(sum(sum(sum(diffpart))))/numel(diffpart);
    if apd1>tol
        sureis=false;
	else
		diffall=abs(diff(imcast(wpict,'double'),1,3));
        apd=sum(sum(sum(sum(diffall))))/numel(diffpart);
        if apd>tol
            sureis=false;
        else
            sureis=true;
