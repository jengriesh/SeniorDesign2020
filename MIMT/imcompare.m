	...
	switch whichimage
		case 'A'
			...
			try
				...
				disp('IMCOMPARE: error evaluating expression for image A. Does it exist?')
				return;
			end
			ns=size(thisimage);
		...
		case 'B'
			...
			try
				...
				disp('IMCOMPARE: error evaluating expression for image B. Does it exist?')
				return;
			end
			ns=size(thisimage);
			
			if numel(ns)~=numel(s) || any(ns~=s)
				disp('IMCOMPARE: Comparing images of different size is not currently supported')
				disp('Image in slot A defines the dimensions required of image in slot B')
				return;
			else
				imageB=thisimage;
			end
...
function generatepadimg()
	...
	padimg=0.5*xor(mod(xx,k*2)<k,mod(yy,k*2)<k)+0.25;
...
function updateranges()
	...
	for c=1:size(imageA,3)
		...
		RMSdiff=(sum(ABerr.^2)./numel(ABerr)).^0.5;
		...
		set(eval(sprintf('handles.diffrange%d',c)),'visible','on')
		set(eval(sprintf('handles.diffrange%d',c)),'string',sprintf('[%4.3E   %4.3E]',diffABc(1),diffABc(2)))

		set(eval(sprintf('handles.diffcount%d',c)),'visible','on')
		set(eval(sprintf('handles.diffcount%d',c)),'string',mat2str([Abigger(c) Asmaller(c)]))

		set(eval(sprintf('handles.RMSdiff%d',c)),'visible','on')
		set(eval(sprintf('handles.RMSdiff%d',c)),'string',num2str(RMSdiff))
		
		set(eval(sprintf('handles.nancount%d',c)),'visible','on')
		set(eval(sprintf('handles.nancount%d',c)),'string',mat2str(nancount))
		
		set(eval(sprintf('handles.oogcount%d',c)),'visible','on')
		set(eval(sprintf('handles.oogcount%d',c)),'string',mat2str([oogA oogB]))
	end
	
	for c=(size(imageA,3)+1):4
		set(eval(sprintf('handles.diffrange%d',c)),'visible','off')
		set(eval(sprintf('handles.diffcount%d',c)),'visible','off')
		set(eval(sprintf('handles.RMSdiff%d',c)),'visible','off')
		set(eval(sprintf('handles.nancount%d',c)),'visible','off')
		set(eval(sprintf('handles.oogcount%d',c)),'visible','off')
	end
end
