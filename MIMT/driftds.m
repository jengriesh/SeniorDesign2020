...
for m=1:1:length(bluramount);
    ...
    if disableagrad == 0 && length(bluramount) > 1;
        agrad=imcrop(agradbase,[L-l*(m-1) 0 [W L]-[1 1]]);
        agrad=imrotate(agrad,180*atan(normdir(2)/normdir(1))/pi,'bilinear','crop');
        agrad=imcrop(agrad,[fliplr(size(agrad)-osize)/2 fliplr(osize)-[1 1]]);
    end
    ...
    for n=1:1:length(amount);
        ...
        wtemp=imcrop(wtemp,[maxdisplacement maxdisplacement fliplr(osize)-[1 1]]); 
