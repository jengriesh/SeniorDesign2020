function outpict=immodify(inpict,varargin)
...
if ~isempty(h)
	...
	msrgb={'adjust color','levels','flip','blur','tonemap','vector means & extrema','blockify','shuffle','glass tiles','dither','erase lines'};
	msbw={'levels','flip','blur','vector means & extrema','blockify','shuffle','glass tiles','dither','erase lines'};
	thismode=1;
	autopreview=1;
	protoimage=[];
	...
	numimchans=size(inpict,3)-hasalpha;

	% parameters
	tweakmodedisplay={'RGB','HSL','HSV','HSI','HSY','HuSL (LAB)','HuSL (LUV)','HSYp','HuSLp (LAB)', ...
		'HuSLp (LUV)','YPbPr (cylindrical)','LCH (CIE LAB)','LCH (CIE LUV)','LCH (SRLAB2)'};
	tweakmodestrings={'rgb','hsl','hsv','hsi','hsy','huslab','husluv','hsyp','huslpab','huslpuv', ...
		'ypbpr','lchab','lchuv','lchsr'};
	cvec=[1 1 1];
	cvecinv=[0 0 0];
	ishue=cvec;
	cmax=2;
	...
	contrastmax=2.5;
	hc=[];
	blurmodestrings={'gaussian','disk','ring','square','motion'};
	selectedblurmode=1;
	blursize=10;
	blurangle=0;
	tmblendmodestrings={'overlay','contrast','softlight','vividlight','glow','heat','helow','gleat'};
	tmblendmode=1;
	tmblendamount=2;
	tmblursize=100;
	tmbluropacity=0.75;
	...
	if numimchans==1
		set(handles.colormodecheckbox,'enable','off')
...
function figuresetup()
	...
	'String','Commit',...
	'TooltipString','<html>Add this layer to the composition and recompose.<br/>Clicking again will update the same layer.<br/>Close window or click ''Modify Layer'' to start a different operation<br/></html>',...
	'Tag','commitbutton',...
	...
	'Units','normalized',...
	'FontSize',10,...
	'Position',[hm vm+8*vp+6*lh+12*bh 0.30 bh],...
	'String','Auto Set',...
	...
	'Style','checkbox',...
	'value',colormode,...
	'TooltipString','when off, vector samples are merged across channels',...
	'Tag','colormodecheckbox',...
	'callback',{@setparam,'colormode'});
...
function setparam(objh,~,param,idx)
	switch param
		case 'orientation'
			orientation=get(objh,'value');
			
		case 'colormode'
			colormode=get(objh,'value');
		...
		case 'chvalslider'
			val=get(objh,'value');
			if ishue(idx)
				set(eval(['handles.ch' num2str(idx) 'box']),'string',num2str(val));
			else
				set(eval(['handles.ch' num2str(idx) 'box']),'string',num2str(abs(val)));
			end
			if cvecinv(idx) == 1
				cvec(idx)=-val;
			else 
		...
		case 'invertchannel'
			cvecinv(idx)=get(eval(['handles.ch' num2str(idx) 'invertcheckbox']),'value');
			cvec(idx)=-cvec(idx);
		...
		case 'levelslider'
			val=get(objh,'value');
			set(eval(['handles.lv' num2str(idx) 'box']),'string',num2str(val));
			lvvec(idx)=val;
			updatelvsliders(idx)
		...
		case 'autolevel'
			...
			lvvec(3:6)=[0 1 1 1];
			for c=1:6
				set(eval(['handles.lv' num2str(c) 'box']),'string',num2str(lvvec(c)));
				updatelvsliders(c)
			end
		...
		case 'selectedblurmode'
			...
			if strcmp(mode,'motion')
				set(handles.bluranglebox,'visible','on')
				set(handles.bluranglelabel,'visible','on')
			else
				set(handles.bluranglebox,'visible','off')
				set(handles.bluranglelabel,'visible','off')
			end
		
		case 'blursize'
			blursize=str2double(get(objh,'string'));
...
function setchannellabels()
	...
	switch thistweakmode
		case {'hsl','hsv','hsi','hsy','hsyp','huslab','huslpab','husluv','huslpuv'}
			set(handles.channel1label,'string','Hue')
			ishue(1)=1;
		case {'lchab','lchuv','lchsr'}
			set(handles.channel1label,'string','Lightness')
			ishue(1)=0;
		case 'rgb'
			set(handles.channel1label,'string','Red')
			ishue(1)=0;
		case 'ypbpr'
			set(handles.channel1label,'string','Luma')
			ishue(1)=0;
	end
	
	switch thistweakmode
		case {'hsl','hsv','hsi','hsy','hsyp','huslab','huslpab','husluv','huslpuv'}
			set(handles.channel2label,'string','Saturation')
		case {'lchab','lchuv','lchsr','ypbpr'}
			set(handles.channel2label,'string','Chroma')
		case 'rgb'
			set(handles.channel2label,'string','Green')
	end
	ishue(2)=0; 
	
	switch thistweakmode
		case {'hsl','huslab','huslpab','husluv','huslpuv'}
			set(handles.channel3label,'string','Lightness')	
			ishue(3)=0;
		case {'hsy','hsyp'}
			set(handles.channel3label,'string','Luma')
			ishue(3)=0;
		case {'lchab','lchuv','lchsr','ypbpr'}
			set(handles.channel3label,'string','Hue')
			ishue(3)=1;
		case 'rgb'
			set(handles.channel3label,'string','Blue')
			ishue(3)=0;
		case 'hsv'
			set(handles.channel3label,'string','Value')
			ishue(3)=0;
		case 'hsi'
			set(handles.channel3label,'string','Intensity')
			ishue(3)=0;
	end
...
function initsliders(channel)
	if ishue(channel)
		...
		set(eval(['handles.ch' num2str(channel) 'minlabel']),'string','-0.5');
		set(eval(['handles.ch' num2str(channel) 'maxlabel']),'string','0.5');
		set(eval(['handles.ch' num2str(channel) 'maxlabel']),'visible','on');
		set(eval(['handles.ch' num2str(channel) 'maxbox']),'visible','off');
		set(eval(['handles.ch' num2str(channel) 'invertcheckbox']),'visible','off');
		set(eval(['handles.ch' num2str(channel) 'slider']),'min',-0.5);
		set(eval(['handles.ch' num2str(channel) 'slider']),'max',0.5);
		set(eval(['handles.ch' num2str(channel) 'slider']),'value',cvec(channel));
	else
		set(eval(['handles.ch' num2str(channel) 'minlabel']),'string','0');
		set(eval(['handles.ch' num2str(channel) 'maxlabel']),'string',num2str(cmax));
		set(eval(['handles.ch' num2str(channel) 'maxlabel']),'visible','off');
		set(eval(['handles.ch' num2str(channel) 'maxbox']),'visible','on');
		set(eval(['handles.ch' num2str(channel) 'maxbox']),'string',num2str(cvecmax(channel)));
		set(eval(['handles.ch' num2str(channel) 'invertcheckbox']),'visible','on');
		set(eval(['handles.ch' num2str(channel) 'slider']),'min',0);
		set(eval(['handles.ch' num2str(channel) 'slider']),'max',cvecmax(channel));
		set(eval(['handles.ch' num2str(channel) 'slider']),'value',abs(cvec(channel)));
	end
	
	set(eval(['handles.ch' num2str(channel) 'invertcheckbox']),'value',cvecinv(channel));
	set(eval(['handles.ch' num2str(channel) 'box']),'string',num2str(cvec(channel)));
end
...
function updatelvsliders(channel)
	set(eval(['handles.lv' num2str(channel) 'slider']),'value',lvvec(channel));

	if channel==5
		set(eval(['handles.lv' num2str(channel) 'slider']),'max',gammamax);
		set(handles.gammamaxbox,'string',num2str(gammamax));
	elseif channel==6
		set(eval(['handles.lv' num2str(channel) 'slider']),'max',contrastmax);
...
function previewbutton_CBF(~,~)
	...
	switch mode
		...
		case 'flip'
			if fliphoriz
				protoimage=flipdim(inpict,2);
			else
				protoimage=inpict;
			end
			if flipvert
				protoimage=flipdim(protoimage,1);
			end
		case 'levels'
			mode=imadjustmodestrings{selectedimadjustmode};
			protoimage=inpict;
		...
		case 'blur'
			mode=blurmodestrings{selectedblurmode};
			protoimage=padarray(inpict,[1 1]*blursize,'replicate');
			switch mode
				case 'gaussian'
					% this replicates the behavior of GIMP with a single size parameter
					sigma=sqrt(-(blursize^2)/(2*log10(1/255)));
					fk=fspecial('gaussian',[1 1]*ceil(blursize),sigma);
				case 'disk'
					fk=fspecial('disk',blursize/2);
				case 'ring'
					w=max(1,round(blursize/5));
					fk=fspecial('disk',blursize/2);
					fk=max(fk-padarray(fspecial('disk',(blursize-2*w)/2),[1 1]*w,0,'both'),0);
					fk=fk/sum(sum(fk));
				case 'square'
					fk=fspecial('average',blursize);
				case 'motion'
					fk=fspecial('motion',blursize,blurangle);
			end
			protoimage=imfilter(protoimage,fk);
			protoimage=cropborder(protoimage,ceil(blursize));
		...
		case 'vector means & extrema'
			protoimage=inpict;
			protoimage(:,:,1:numimchans)=meanlines(inpict(:,:,1:numimchans),orientation,colormode, ...
		...
		case 'shuffle'
			protoimage=inpict;
			protoimage(:,:,1:numimchans)=shuffle(protoimage(:,:,1:numimchans),shuffletiles,shufflergb);
		case 'glass tiles'
			protoimage=inpict;
		...
		case 'erase lines'
			protoimage=inpict;
			protoimage(:,:,1:numimchans)=eoline(protoimage(:,:,1:numimchans),eoldir,eolratio,eoloffset);
	end
...
function safepict=alphize(thispict)
	...
	if hasalpha
		...
		mask=thispict(:,:,end);
		safepict=bsxfun(@plus,mat.*(1-mask), ...
			bsxfun(@times,thispict(:,:,1:end-1),mask));	
	else
		safepict=thispict;
	end
end
