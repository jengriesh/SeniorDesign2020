...
for p=1:passes
	...
	if strcmp(tftype{p},'scale') 
		...
		for c=1:cycles(p)
			...
			dblpict=imresize(dblpict,scalevec,thismethod);
			intpict=imresize(intpict,scalevec,thismethod);
		end
		
		% remove cumulative scaling
		dblpict=imresize(dblpict,s,thismethod);
		intpict=imresize(intpict,s,thismethod);
	...
	elseif strcmp(tftype{p},'rotate')
		...
		for c=1:cycles(p)
			...
			% rotate cycle
			dblpict=imrotate(dblpict,st(p,1),thismethod,'crop');
			intpict=imrotate(intpict,st(p,1),thismethod,'crop');
		end
		
		% remove cumulative rotation
		dblpict=imrotate(dblpict,-displacement,thismethod,'crop');
		intpict=imrotate(intpict,-displacement,thismethod,'crop');
		% crop back to size
