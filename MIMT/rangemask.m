...
if numel(channel)~=size(range,1)
    disp('RANGEMASK: number of channels must match number of range vectors');
    return
end
...
if strcmpi(compmode,'not') && length(channel)>1
    disp('RANGEMASK: use ''nand'' instead of ''not'' for the union of negated matches')
    return 
end
...
for t=1:length(channel)
    ...
    for f=1:numframes;
        ...
        switch tidyname
            ...
            otherwise
                disp('RANGEMASK: unknown channel name');
                return
        end
