...
for gl=1:levels
	...
	if (mod(gl,2) || honly) && ~vonly
		if k==0
			if regular
				mh=orddither(imresize(mr,[s(1) 10]));
			else
				mh=zfdither(imresize(mr,[s(1) 10]));
			end
			mh=imresize(mh(:,5),s);
		else
			mh=imresize(zfdither(imresize(mr,[s(1) s(2)/k])),s,'nearest');
		end
	else
		if k==0
			if regular
				mh=orddither(imresize(mr,[10 s(2)]));
			else
				mh=zfdither(imresize(mr,[10 s(2)]));
			end
			mh=imresize(mh(5,:),s);
		else
			mh=imresize(zfdither(imresize(mr,[s(1)/k s(2)])),s,'nearest');
		end
	end
